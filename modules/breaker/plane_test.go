...
func TestPlaneJudgeReportsOpenAndSpared(t *testing.T) {
	t.Run("连到阈值就摘牌", func(t *testing.T) {
		...
		if !strings.Contains(second.Note, "熔断器已打开") {
	...
	t.Run("诊断探活拦下来就不摘", func(t *testing.T) {
		...
		// 所以这行必须自己解释清楚。
		if !strings.Contains(second.Note, "仍然可用") {
...
// 理由。
//
// 理由文案（"熔断中"）以前写死在 modules/config/resolve/chain.go 里——准入的
// 判据属于提供它的人，resolve 只认「返回一个理由」这个形状。它现在回家了，
// 而 `newgate tier` 里那句「为什么不是我想的那个」一个字都没变。
func TestPlaneAdmitIsAvailablePlusAReason(t *testing.T) {
	...
	if ok, why := p.Admit("p", "m"); ok || why != "熔断中" {
		t.Errorf("摘牌后应该不可用且理由是「熔断中」，得到 (%v, %q)", ok, why)
	}
...
func TestPlaneDocKeepsTheWireKey(t *testing.T) {
	...
	if !strings.Contains(string(raw), `"open":true`) ||
		!strings.Contains(string(raw), `"rule":"配置"`) {
...
func TestPlaneObserveProbesIsAuthoritative(t *testing.T) {
	...
	if !strings.Contains(acks[0].Note, "至少 60s") {
...
func TestPlaneMetricNamingLivesHere(t *testing.T) {
	...
	for _, key := range []string{"breaker.opened", "breaker.spared", "breaker.skipped.shape_error"} {
		if g := p.MetricGroup(key); g != "熔断" {
			t.Errorf("MetricGroup(%q) = %q, want 熔断", key, g)
		}
	...
	if g := p.MetricGroup("chain.failover"); g != "" {
		t.Errorf("MetricGroup 认领了不属于它的计数器: %q", g)
	}
