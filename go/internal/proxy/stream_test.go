...
func streamThroughProxy(t *testing.T, upstreamURL string, _ time.Duration) (string, []time.Duration) {
	...
	testChain = func(role string) []config.Step {
		if role != "heavy" {
			return nil
		}
		return []config.Step{{
			Profile:  "test",
			Binding:  config.Binding{Provider: "test-prov", Model: "real-model-1"},
...
func testProvider(baseURL string) config.Provider {
	return config.Provider{BaseURL: baseURL + "/v1", APIKey: "sk-real", Protocol: "openai"}
}
...
func TestNonStreamResponseByteFaithful(t *testing.T) {
	...
	testChain = func(role string) []config.Step {
		return []config.Step{{Profile: "test",
			Binding:  config.Binding{Provider: "p", Model: "real-model-1"},
			Provider: testProvider(up.URL)}}
	}
	defer func() { testChain = nil }()
