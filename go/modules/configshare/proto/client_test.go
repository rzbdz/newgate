...
func TestPullAppliesSnapshot(t *testing.T) {
	...
	if v, ok := LookupSecret(paths.SecretsFile(), "smt-deepseek"); !ok || v != "sk-1" {
...
func TestPullSecretsRotation(t *testing.T) {
	...
	for prov, want := range map[string]string{"smt-deepseek": "sk-2", "glm": "sk-3"} {
		if v, ok := LookupSecret(paths.SecretsFile(), prov); !ok || v != want {
...
func TestPullSecretsDriftRefused(t *testing.T) {
	...
	if _, err := MaterializeSecrets(paths.SecretsFile(), map[string]string{"smt-deepseek": "sk-本地手改的"}); err != nil {
		t.Fatal(err)
	}
	...
	if v, _ := LookupSecret(paths.SecretsFile(), "smt-deepseek"); v != "sk-本地手改的" {
