...
// Ledger 是**账本**：源语言里有哪些消息，各带什么实参、什么复数形式。
//
// 它是生成物（`tools/i18n extract` 写、`-check` 校验），用途是**统计与校对**：
// 覆盖率的分母、占位符一致性、孤儿键检测、给翻译工具的输入。运行时也嵌一份
// （小），这样 doctor 能直接报「翻了多少、还缺多少」，而不必去读仓库。
//
// # 为什么不记源码位置了（2026-09-21 删掉的那一格）
//
// 它曾经有一格 `Where`（`文件:行`），而那笔账算下来是纯亏：
//
//   - **没有任何读者**。运行期唯一碰它的是语言体检那一处，而那里拿到条目之后
//     写的是 `_ = le`——它要的只是「这个 id 在不在账本里」。`tools/i18n check`
//     报错时用的是**当场扫描**出来的位置，不是账本里那份。翻译工具也不用。
//   - **它是唯一让账本天天过期的原因**。`check` 逐字节比对账本，而在任何一个
//     含 `i18n.T(...)` 的文件里加一行注释、挪一个函数，行号就变、文件就不同、
//     整个构建变红——**消息一个字都没改**。用户的原话是「每次改一点代码他就要
//     消耗大量的 token 去修复」，说的就是这一笔：修的不是文案，是行号。
//
// 删掉之后账本只在**消息真的变了**的时候才变：加一条、删一条、改措辞、改实参。
// 那才是需要有人看一眼的时刻（多半还要补译文），而不是每次敲回车。
type Ledger struct {
