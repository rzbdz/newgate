...
做路由、fallback、上游怪癖修补。设计思想见 `docs/16-core-thesis.md`，架构见
`docs/02-architecture.md`。**改代码前先读这两篇**——这个仓库的注释和文档密度
...
分层（依赖单向，`core` 最内层、不 import 任何 IO）：
...
`mid` 顶上。改档位相关的东西，先看 `docs/18-role-preference.md §1.1`。

---

## 2. 测试与验证

- **单测不出网**（`docs/17` §1）：一律用 `httptest`，出网即失败。
...
- **`newgate probe` 有盲区**：主探活只打 provider **声明协议**那条路。
  「probe 全绿、真实流量 404」通常是方言/base 不对（见 `docs/08` §3.1 的
