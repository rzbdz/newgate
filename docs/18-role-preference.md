...
| | 熔断器（已实现） | 屏蔽（本节） |
| --- | --- | --- |
| 触发 | 真实流量连续失败 N 次；probe 失败或极小请求过慢 | **人工 ack** |
| 时长 | 至少 60s，之后须成功 probe 才恢复 | 小时到天 |
| 依据 | 真实流量失败；主动最小请求的可用性 | probe 结果 + 人的判断 |
| 记录 | daemon 全局内存，按 `(provider, model)` | 落盘，带 reason |
| 目的 | 别撞死的上游 | 表达「这个我暂时不想用」 |

熔断是反射，屏蔽是决策。所以屏蔽必须要 ack——**自动屏蔽 4 小时是危险的**：一次偶发 429 就把一个好模型摘掉四小时，用户还不知道为什么。

### probe 对短熔断是判决，对长期屏蔽仍只是证据

probe 打的是 4 token 的最小请求。连接失败、非 200，或耗时超过
`timeouts.classifier_first_byte_ms`（默认 12s），说明它不具备进入交互
fallback 链的资格：结果立即写入 daemon 的全局 binding 熔断表。先隔离 60s，
之后仍须成功 probe 才能恢复。
`newgate tier` 从 daemon 读取同一张表，因此立刻反映实际运行链。

60s 是最短隔离期，不是自动复活倒计时。到期后 binding 仍不回链；只有新的
成功 probe 才能清掉熔断。普通真实请求即使有一条在途请求随后返回 200，也
不能绕过这个门槛。

普通真实请求不能照搬这个 12s：大上下文 prefill 合法地更慢。真实流量仍按
请求身份使用 `classifier_first_byte_ms` 或 `first_byte_ms`，只有触发相应
超时/错误才累计熔断。probe 的延迟只计算主探活，不包含后续方言和 quirk
附加请求。

常规 probe 对每个唯一 `(provider, model)` 只发一次主请求；主请求超时才
重试一次。方言和 quirk 只在该 binding 尚未学习时附加探测，结果写入
`probe-capabilities.json`，以后 probe 与 daemon 启动直接复用，不再重复
...
最近变慢的备用模型也会自然后移。排序复杂度 O(n log n)，当前链规模下可以
忽略。

长期屏蔽仍然需要人工 ack；一次 probe 不会自动写入小时级 disabled 记录。
