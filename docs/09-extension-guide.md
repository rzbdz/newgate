...
**界面不是入口的寄存处**（2026-09-20 改）：守护进程本体（`newgate __serve`）原来
...
## 7. 数据面策略（gateway 的四个决策点）

网关是**最小系统**：它按自己的状态机切出「必须做个决定」的四个地方，各留一个口
和一本注册表，然后就不管了。谁都不往里注册时它照常转发——可用性全放行、按机制
决定沿不沿链走、不记账、不落盘。

| 口 | 数据面此刻的问题 | 可选接口 |
| --- | --- | --- |
| A 建链期 | 这条 binding 现在能不能进候选？排第几？ | `Admitter` |
| B 结局期 | 这一发算谁的账？留什么痕？记哪些计数器？ | `Adjudicator` |
| C 控制面 | 我自己那份状态怎么报出去？探活结论怎么收？ | `Controller` |
| D 停机 | 关机前我这边还有谁的账没落盘？ | `Flusher` |

另有 `EnvBinder`（接运行期能力：日志出口、一次最小探活）与 `MetricNamer`（认领
自己那几个计数器在 `newgate metrics` 里的分组与说明）。

一个贡献者是一个 `policy.Filter`（`Name` + `Why`），按需要实现其中任意几个接口；
注册走 `gateway.RegisterFilter`，在**自己的 `Start`** 里做，拿到的 `Release` 在
`Stop` 里撤（见 §3 的规矩）。
...
- **策略只能叫停，不能强行前进**。机制默认 `advance = !written && !last`，
  `Verdict.Stop` 的作用只是把它翻成 false。不存在「策略要前进而机制要停」这种
  组合，所以 `Stop` 这一位永远不会让内核处于自相矛盾的状态。
- **准入者只允许一个**（注册第二个会当场报错）。理由不是保守：`Admit` **不是
  查询**——健康表在冷却期满时会发放「半开试探名额」，一个名额只放行一次真实
  请求，而 resolve 对每个候选恰好问一次。多个准入者求 AND 会让「名额发不发」
  取决于另一个不相关贡献者的返回值：前者发了名额、后者否决，那条 binding 就
  在 TTL 内既进不了链、也没人在试。
...
要**全关**（「这个发行版只要框架 + 我自己的模块」）**不要列一长串目录名**，用
`AllCore`：

```go
app.Selection{AllCore: true, Extra: []app.Entry{{Dir: "hello", Component: hello.New()}}}
```

发行版的规格书里写 `"disable": ["*"]`，`tools/distgen` 把它编成 `AllCore: true`。
列满名字的写法带一份**会腐坏的副本**：内核往 `CoreModules()` 里加一个模块时，那份
名单不会跟着长，于是「我以为全关掉了」的发行版悄悄多装了一个模块——而且不报错。
`AllCore` 跟着 `CoreModules()` 一起长，**关不掉的那个自动留在外面**（见下一段的
边界），所以它同时是「全关」和「关得掉的全关」这两句话的同一份实现。
`AllCore` 与 `Disable` 同时给会报错：两个说法说同一件事时，后者只会变成前者的过时副本。
...
模块的用户可见文本**不许写在源码里**——源码里写英文，调
`i18n.T("English {x}", i18n.A{"x": …})`（叶子包 `.../lib/i18n`）；译文放进自己的
`<模块>/locale/catalogs/<tag>.json`，在 `Start` 里用 `go:embed` + 注册交给
`modules/locale` 的端口。**谁的东西谁带**：摘掉模块，它的译文跟着消失；装哪些
语言最终还是规格书里的取舍（发行版点名哪些模块）。

规矩与「为什么」见 [docs/13-i18n.md](13-i18n.md)；两个仓库共用一套静态检查
（`tools/i18n/{scan,check}`），所以内核与发行版的判断不会分叉。
