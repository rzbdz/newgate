...
以上约束的是「包装启动」路径。控制命令（`status` / `tier` / `doctor` / `metrics` / `omo` …）
的输出就是它的全部产出，另有约定，见 §5.1。
...
| 约定 | 为什么 |
| --- | --- |
| 宽度一律走 `style.Width` / `Pad`，不用 `%-22s` | `%` 系列按**字节**补齐；中文一个字 3 字节、显示占 2 列，中英混排的表必然错位 |
| 控制命令每个物理行最多 75 列 | 给窄终端和分屏留余量；模型名、URL、槽位键不从中间硬拆，列放不下就切换纵向卡片 |
| 颜色只在 stdout 是终端时输出 | `newgate status > 报告.txt` 不该把 `\033[1m` 写进文件。判据顺序：`NEWGATE_COLOR=always\|never` → `NO_COLOR` → 字符设备 |
| 状态标记只有四个：`✓` 做到 / `✗` 出错 / `·` 有意跳过 / `⚠` 能用但不对 | 标记一多，「有意的跳过」就会被读成「出错」 |
| 结论一行、明细缩进、解释走 `Hint`（暗色） | 扫读时自动跳过解释；出事时才需要读它 |
| 陈述句、无主语、带单位、无语气词 | 「别担心」「其实」「我们」不出现；`120000ms` 写成 `2m0s` |
| 默认视图只给结论，明细按需展开 | `newgate tier` 曾经把 106 条跳过原因逐条铺开，其中 91 条是必然发生的去重——把结论（走谁）淹掉了 |
| 不展示与用户判断无关的运行时杂质 | shim 目录里躺着一个 `claude-bak` 既不影响功能也不影响判断，报出来只是噪音 |

`tier` 的两档视图是这套约定的样板：不带参数按档位分组，每条 fallback 链
在候选边界纵向排列；带档位名则展开成编号的站与按原因分组的跳过计数。
模型标识符完整显示，不用省略号隐藏候选，也不在标识符中间硬折。
档位名支持前缀（`newgate tier h`），也接受动态角色键（`newgate tier omo-sisyphus`，
见 §4 与 docs/18）——模块注册的槽位和档位在解析层是一回事，命令层不该比解析层更窄。
...
分页器只是显示手段，起不来就直接打印——不能因为没装 `less` 就看不到日志。

`st`（special_treatment）同理：清单里每个插件一行「为什么存在」，完整说明走
`newgate st <插件>`。这一层会改用户的请求，清单必须一眼扫完。
