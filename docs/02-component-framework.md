...
### 三个时刻：Bind / Start / Serve——以及那条最常被违反的规矩

「装配只有一个阶段」说的是**没有第二次 `Start`**，不是「`Start` 一来就能服务
了」。把一个进程的生命切成三段，好几个看着像环的东西会自己散开：

| 时刻 | 谁在跑 | 保证 |
| --- | --- | --- |
| **Bind** | 没有人（框架在构图） | 每个端口的**存储**已经就位。`Provides` 在任何 `Start` 之前全部绑好——界面那七本账（命令 / 状态行 / 体检 / 诊断包 / 术语 / 开关 / 详细模式）在界面的 `Start` 之前就存在了 |
| **Start** | 各组件自己的代码，按拓扑序 | 硬依赖的提供者已经 `Start`；要往哪个 owner 注册，那个 owner 已经 `Start` |
| **Serve** | 执行期：分派命令、处理请求、拉起客户端 | 全部 `Start` 都已返回 ⇒ **每本注册表必然是完整的** |

由此得到一条规矩（比 §1.5 的「需要被启动吗」更常被违反）：

> **`Start` 只做两件事：起自己的服务、往 owner 注册自己的贡献。
> 它不得把「别人注册了什么」当作自己启动的输入。**

**为什么依赖图救不了它**：`runtime` 依赖的是 `confighook`（owner），而往目录里写
agent 的是 `claudecode` / `opencode`（贡献者）。依赖图能表达「我要 owner 起来」，
...
**「让 owner 依赖它的注入者」是错的解法**，它会同时破坏三件事：不解决那个 bug
（要等的是贡献者，不是 owner；把界面挪到最后，`Start` 里那次读的时刻一点没变）、
破坏「撤注册时 owner 还活着」（owner 先停，注册者随后往一具尸体上 `Release`）、
什么也没买到（owner 的 `Start` 若是空的，它本来就不需要「所有人都写完」）。这个
提议指向的真问题是「注册表有写侧和读侧两个面，而依赖边只表达『谁先起来』」——
结论应该是「**读侧不许在 `Start` 期读**」，不是「把 owner 挪到最后」。

真到了「必须拿到完整目录才能构造自己的服务」那天，要加的也**不是** per-component
的第三阶段（那就是被删掉的 `Attach` 的幽灵），而是 **owner 侧的封口点**：组合根在
全部 `Start` 返回之后驱动一次 `Seal`，owner 据此建索引 / 校验 / 算默认值。契约只有
一条：**`Seal` 里只读不注册**。判据是「有没有第二个真实案例」——今天没有
（`pluginmanager.SetCatalog` 由组合根手调，`breaker` 的读全在请求期），所以不建。
